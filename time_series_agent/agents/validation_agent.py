...
class ValidationAgent:
    ...
    def __init__(self, model: str = "gpt-4o", config: dict = None):
        ...
        self.available_models = model_cfg.get("available_models", self.config.get("available_models", []))
        self.n_candidates = int(model_cfg.get("n_candidates", self.config.get("n_candidates", 5)))
        self.cv_folds = int(model_cfg.get("cross_validation_folds", self.config.get("cv_folds", 5)))
        self.k_models = int(self.config.get("k_models", model_cfg.get("k_models", 3)))
        self.optimization_method = model_cfg.get("optimization_method", self.config.get("optimization_method", "grid_search"))
        self.max_hpo_combinations = int(
            self.config.get("hpo_max_combinations", model_cfg.get("hpo_max_combinations", 20))
        )
        self.hpo_patience = int(self.config.get("hpo_patience", model_cfg.get("hpo_patience", 5)))
    ...
    def run(self, analysis_result: str, available_models: List[str], validation_data: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not model_selection or not model_selection.get('selected_models'):
                ...
                fallback_selection = []
                for model_name in available_models[:self.k_models]:
                    fallback_selection.append(
                        {
                            "model": model_name,
                            "hyperparameters": {},
                            "reason": "Fallback due to model selection parsing failure",
                        }
                    )
                model_selection = {
                    'selected_models': fallback_selection,
    ...
    def _optimize_model_hyperparameters(self, validation_data: pd.DataFrame, 
        ...
        param_values = [values if isinstance(values, list) else [values] for values in hyperparameters.values()]
        ...
        for combination in product(*param_values):
            ...
            if len(combinations) >= self.max_hpo_combinations:
                break
        ...
        for i, params in enumerate(combinations):
            try:
                ...
                logger.info(
                    "HPO trial %s/%s for %s: MSE=%.4f MAE=%.4f MAPE=%.2f%% params=%s",
                    i + 1,
                    len(combinations),
                    model_name,
                    metrics['mse'],
                ...
                if self.hpo_patience > 0 and no_improvement_rounds >= self.hpo_patience:
                    logger.info(
                        "HPO early stopping for %s after %s non-improving trials",
                        model_name,
        ...
        logger.info(
            "Best hyperparameters for %s: %s (MAE=%.4f)",
            model_name,
            best_params,
            best_metrics['mae'],
        )
